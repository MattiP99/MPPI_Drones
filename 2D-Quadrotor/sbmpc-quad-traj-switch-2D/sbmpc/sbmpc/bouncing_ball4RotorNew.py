...
r_b = np.array([0.0, 9.1])  # ball position (x_b, y_b)
v_b = np.array([0.0, 0.0])  # ball velocity (v_bx, v_by)

# Input force from the drone's rotors to hover and fly horizontally
def input_force(t):
    F_vertical = np.array([0.0, (m_d+ m_b) * g])  # Vertical force to counteract gravity
    #F_horizontal = np.array([1.0, 0.0])    # Horizontal force to move right
...
def equations_of_motion(t, state):
    ...
    if dist > L:
        ...
        # Calculate drone's acceleration
...
def handle_collision(r_d, v_d, r_b, v_b, L, m_d, m_b):
    ...
    if dist > L:
        ...
        direction = (r_b - r_d) / dist
        relative_velocity = ( v_b - v_d) / dist

...
class SbMPC:
    ...
    """
    def __init__(self, model: BaseModel, objective: BaseObjective,  config_mpc: ConfigMPC, config_general: ConfigGeneral): #, base_helper: base_helper.Helper):
        """
        ...
        #self.iter = base_helper.iter_helper
    ...
    def compute_rollout_batched(self, initial_state, reference, control_variables):
        ...
        #for idx in range(self.iter, self.iter + self.horizon):
        ...
        #cost += self.final_cost(curr_state, reference[self.iter + self.horizon + 1, :])
        cost += self.final_cost(curr_state, reference[self.horizon + 1, :])

        return cost

    def _rollout_step(self, state, reference, control_variables, idx):
        #jax.debug.print("{control_variables}", control_variables = control_variables)
    ...
    def compute_control_mppi(self, state, reference, best_control_vars, key):
        ...
        #jax.debug.print("{best_control_vars}", best_control_vars = best_control_vars)

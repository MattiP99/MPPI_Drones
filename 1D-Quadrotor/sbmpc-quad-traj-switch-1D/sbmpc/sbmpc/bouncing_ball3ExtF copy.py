...
# Input force acting on the drone
F_d = np.array([0.5, 0.0])
#F_d = np.array([0.0, (m_d + m_b)*g])  # Modify this as needed
...
def equations_of_motion(t, state):
    ...
    if dist > L:
        ...
        # Project drone's force along the direction of the cable
        F_d_proj = np.dot(F_d, direction) * direction
        a_d = F_d / m_d + F_d_proj / m_b
    else:
        # Cable is slack
        a_d = F_d / m_d  # Only external force acts on the drone
        a_b = np.array([0, -g])  # Only gravity acts on the ball
